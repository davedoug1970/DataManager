//
//  PersonRealmDetailViewController.swift
//  DataManager
//
...
class PersonRealmDetailViewController: UIViewController {
    ...
    init?(coder: NSCoder, person: PersonRM?) {
        self.person = person
    ...
    override func viewDidLoad() {
        ...
        if let editPerson = person {
            ...
            if let address = editPerson.address {
                streetAddressTextField.text = address.streetAddress
            ...
            title = "Edit Person (Realm)"
        } else {
            title = "Add Person (Realm)"
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if person != nil {
            ...
            var gender: GenderRM = .male
            
            if genderTextField.text!.lowercased() == "female" {
                gender = .female
            }
            
            let tempAddress = AddressRM(streetAddress: streetAddressTextField.text!, city: cityTextField.text!, state: stateTextField.text!, postalCode: postalCodeTextField.text!)
            
            var phoneType: PhoneTypeRM = .home
            
            if phoneTypeTextField.text!.lowercased() == "work" {
                phoneType = .work
            }
            
            let tempPhoneNumbers = List<PhoneNumberRM>()
            tempPhoneNumbers.append(PhoneNumberRM(type: phoneType, number: phoneNumberTextField.text!))
            
            person = PersonRM(id: ObjectId.generate(), firstName: firstNameTextField.text!, lastName: lastNameTextField.text!, gender: gender, age: Int(ageTextField.text!)!, address: tempAddress, phoneNumbers: tempPhoneNumbers)
        }
    }
}
